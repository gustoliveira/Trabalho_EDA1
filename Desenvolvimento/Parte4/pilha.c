...
struct no{
    unsigned long int codCliente, qntOperacao;
    long int valor;
    no* pai;
};
...
unsigned long int codCliente(no *n){
	return n->codCliente;
}
...
no* search_p(pilha* p, unsigned long int codCliente){
  ...
  if(paux !=NULL){
    ...
    if (isEmpty_p(p)==0){
      while(p->cont>0){
        no* aux = pop_p(p);
        if(aux->codCliente == codCliente && flag == 0){
          resultado = aux;
          flag = 1;
        }
        push_p(paux,aux);
